...
namespace JobPortalWebAPI.Repositories
{
    public interface IUserJobRepository
    {
        ...
        Task<PaginatedResults<ReturnJobDTO>> GetAllSavedJobsAsync(string userId, int pageNumber);
        ...
        Task<PaginatedResults<ReturnAppliedJobsDTO>> GetAllAppliedJobsAsync(string userId, int pageNumber);
        ...
        Task<PaginatedResults<ReturnJobDTO>> GetJobsAsync(string? searchQuery, string? jobLocation, string? jobCategory, string? jobLevel, string? jobType,  int pageNumber);
    }
}
